(ns platts-api.main
  "Kotodama WASM entrypoint for the Platts_api clean-room actor (L4) — Clojure port.
  ...
  facts (`platts_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "platts_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"platts_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "platts_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :platts_api-compat/ready)
